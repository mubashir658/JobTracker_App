...
export default function LandingPage({
  ...
  setLandingPage: propSetLandingPage,
  onLoginClick,
  onSignupClick,
}) {
  const [localDark, setLocalDark] = useState(propDark ?? true);
  const [localPage, setLocalPage] = useState(propLandingPage ?? "home");
  const [feedback, setFeedback] = useState(FEEDBACK_INIT);
  const [submitted, setSubmitted] = useState(false);

  const dark = propSetDark ? propDark : localDark;
  const setDark = propSetDark || setLocalDark;

  const page = propSetLandingPage ? propLandingPage : localPage;
  const setPage = propSetLandingPage || setLocalPage;
    ...
    <div style={{ minHeight: "100vh", background: bg, color: txt, fontFamily: "'Segoe UI', system-ui, sans-serif", transition: "all 0.2s" }}>
        ...
        <div>
          ...
          <div style={{ textAlign: "center", padding: "80px 24px 60px", maxWidth: 720, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={() => onSignupClick ? onSignupClick() : null}
      ...
      <footer style={{ borderTop: `1px solid ${border}`, padding: "28px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, marginTop: 20 }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <a href="https://github.com/mubashir658/JobTracker_App" target="_blank" rel="noreferrer" style={{ fontSize: 12, color: muted, textDecoration: "none" }}>GitHub</a>
          <a href="https://www.linkedin.com/in/mohammed-mubashir-ali-hyd658" target="_blank" rel="noreferrer" style={{ fontSize: 12, color: muted, textDecoration: "none" }}>LinkedIn</a>
        </div>
      </footer>
